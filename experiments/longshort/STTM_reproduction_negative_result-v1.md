...
Воспроизведён метод **STTM** (Stock-Tonal Topic Modeling; Рябых и др., PeerJ CS 2022) на корпусе новостей «Коммерсантъ» 2013–2021 и ценах 39 тикеров MOEX TQBR. Формальное воспроизведение успешно: **gross Sharpe 1.217 (статья: 1.37 ± 0.09)**, net Sharpe 1.198 при комиссии 0.05% и 1.160 при 0.15%. Однако **четыре независимых теста показывают, что этот Sharpe — не альфа, а артефакт кросс-секционного отбора**: (1) плацебо-тест с перетасованными доходностями даёт Sharpe 1.48 ± 0.36 (z = −0.16, p = 0.50, **не значимо**); (2) эндогенный базовый уровень AR(5) по цене даёт ту же accuracy (0.516), что и STTM (0.551), и лучший Spearman ρ (+0.068 против −0.054); (3) out-of-sample тест 2022–2026 даёт Sharpe −0.23 (MaxDD −45.5%, direction accuracy 46.2% — хуже монетки); (4) **новый long-short decile тест (top-20% long vs bottom-20% short, dollar-neutral) даёт net Sharpe 0.064 после издержек — статистически неотличим от placebo (p = 0.68, z = −0.59).** Мы делаем вывод, что новостная компонента не добавляет предиктивной силы поверх базовой рыночной экспозиции, и публикуем это как **honest negative result**. Проект служит воспроизводимой реализацией метода с полным набором диагностических тестов, применимой для проверки аналогичных гипотез в будущем.
...
**Результат** (из `data/processed/placebo_kommersant.json`):
...
**Интерпретация:** реальный Sharpe (1.217) **ниже** медианы placebo. z = −0.71 означает, что реальный результат находится на 0.71 стандартного отклонения ниже среднего случайного. p=0.76 — нулевая гипотеза «STTM не лучше случайного» **не отвергается**. Sharpe 1.217 — это **свойство кросс-секционного отбора**, а не сигнал из новостей.

**Ключевой механизм** (из `findings/placebo_kommersant_2026-08-25.md`): даже на перетасованных доходностях $f_{\text{topics}}$ пересчитывается на train-годах с перетасованными returns. Корреляции «слово ↔ доходность» случайны, но **не нулевы** (39 тикеров × 200 недель = много степеней свободы). STTM-индекс строится из этих случайных корреляций → сигнал → кросс-секционный отбор → **Sharpe ~1.4 на растущем рынке MOEX 2015-2021**.
...
| Long-only placebo | IS 2015-2021 | STTM (shuffled) | 1.476 ± 0.36 | **0.76** | NOT SIGNIFICANT ❌ |
...
**Все четыре независимых теста указывают в одну сторону: STTM не даёт альфы.**
...
- ❌ **STTM не даёт статистически значимой альфы** (placebo p=0.50)
...
**Long-only top-20%** на растущем рынке имеет положительный expected return **независимо от качества сигнала**. Это эмпирический факт для MOEX 2015-2021 (бычий рынок + ретейл-доминирование + моментум-эффект). На placebo (per-column shuffle) этот эффект **сохраняется**, потому что shuffling не разрушает cross-sectional ranking — он лишь разрушает связь «сигнал → доходность». Поэтому placebo mean = 1.48.
...
Это объясняет:
1. Placebo Sharpe = real Sharpe (кросс-секционный отбор работает на любой частоте)
...
#### 5.3.4. Vocabulary drift

Между 2013 и 2026 — два поколения токенов:
- 2013–2019: «санкции», «Крым», «нефть», «рубль», «ЦБ»
- 2020–2026: «пандемия», «COVID», «льготная ипотека», «ключевая ставка»

Словарь, обученный на 2013–2021, **неизбежно хуже работает** на 2022–2026 (хотя в нашем тесте vocabulary drift — вторичная проблема: даже с обновлённым словарём OOS Sharpe отрицательный).
...
5. **Нет walk-forward с purged CV** (de Prado, 2018). Наш expanding CV имеет look-ahead risk на стыках годов.
...
# Тесты
python -m pytest tests/ -q                   # 70/70 pass
